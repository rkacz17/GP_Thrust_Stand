...
// TWEAK PICK AND PLACE FUNCTIONS FOR CALIBRATION ADJUSTMENT
...
void loop() {
  ...
  switch (state) {
    ...
    case 2:  // run Calibration sequence
      ...
      place(); // STEPPER SERVO PLACE FUNCTION (places weight)
      delay(9000);
      FS_cali = (53.812 * 9.81) / ((analogRead(FS_pin) - FS_zero) * bit_resolution);  // Calibration Value with Weight
      delay(1000);
      pick(); // STEPPER SERVO PICK FUNCTION (grabs weight)
      delay(1000);
      state = 3;
      break;
